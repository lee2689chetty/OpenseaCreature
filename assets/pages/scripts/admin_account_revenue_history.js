...
var AjaxCommunication = function () {
    var initAjaxCom = function(){
        $('#btGenerate').on('click', function(){
            ...
            formData.append("currency", jQuery('#currencyType').val());
    ...
    var UpdateSystemNecessaryViews = function(jsonValue){
        ...
        for(var i = 0 ; i < jsonValue.transList.length; i++)
        {
            ...
            {
                CreditOrDebit = '<td style="padding-top: 15px; padding-bottom: 15px;"> <span> <p class="font-green"> ' + valueTmp.amount.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") + ' </p> </span> </td>';
            }
            else
            {
                CreditOrDebit = '<td style="padding-top: 15px; padding-bottom: 15px;"> <span> <p class="font-red"> ' + valueTmp.amount.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") + ' </p> </span> </td>';
            }

            console.log(valueTmp);
            var transferId = '<a href="../../request/status/' + valueTmp.transaction_id + '" class="btn btn-link">' + '#' + valueTmp.transaction_id +'</a>';
            var account = '<a href="../../account/detail/' + valueTmp.account_id + '" class="btn btn-link">' +  valueTmp.account_number +'</a>';
            table.fnAddData([
                ...
                '<td style="padding-top: 15px; padding-bottom: 15px;"> '+ account +' </td>',
                '<td style="padding-top: 15px; padding-bottom: 15px;"> '+ valueTmp.account_type +' </td>',
                '<td style="padding-top: 15px; padding-bottom: 15px;"> ' + transferId + '</td>',
                ...
                '<td style="padding-top: 15px; padding-bottom: 15px;"> ' + valueTmp.current.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") + '</td>',
